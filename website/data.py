...
from . import db
from .models import User, Expense, Budget, Investment, Stock_and_Crypto, Custom_Investment, Bill, Goal
    ...
    for user in user_ids:
        ...
        for i in range(num_of_goals):
            ...
            if goal_name == "Buying a Lamborghini":
                goal_description = choice(["Only 47 Lamborghinis in my Lamborghini account", ""])
            elif goal_name == "Reading a book":
                goal_description = choice(["Like Book", ""])
            elif goal_name == "Hosting a Ted X Talk":
                goal_description = choice(["Where I talk about my Lamborghini account", ""])
            elif goal_name == "Creating Hollywood Hills Account":
                goal_description = choice(["Only 47 Hills in my Hollywood Hills account", ""])
            elif goal_name == "Buying a basketball court":
                goal_description = choice(["Tennis, Basketball, tennis, basketball", ""])
                
            goal_date = date.today() + timedelta(days=randint(1, 365 * 3))
            #total_contribution = uniform(0.00, goal_amount)
            contribution_frequency = choice(["Weekly", "Fortnightly", "Monthly"])
...
def add_stock_and_crypto_data():
    ...
    """
    user_ids = generate_user_list()
    stock_names = ["Tesla", "Apple", "Microsoft", "Amazon", "NVIDIA", "VISA", "Meta Platforms"]
    crypto_names = ["Bitcoin", "Ethereum", "Litecoin", "Ripple", "Cardano", "Dogecoin", "Solana"]
...
def add_custom_investment_data():
    ...
    for user in user_ids:
        ...
        for i in range(num_of_custom_investments):
            ...
            sold_date = choice([None, date.today() - timedelta(days=randint(1, 60))])
            if sold_date is not None:
                purchased_date = str(sold_date - timedelta(days=randint(1, 60)))
            else:
                purchased_date = str(date.today() - timedelta(days=randint(1, 60)))
